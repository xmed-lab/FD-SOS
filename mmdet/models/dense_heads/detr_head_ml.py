...
class DETRHeadML(BaseModule):
    ...
    def __init__(
        ...
        self.attribute_classes = 2
        self.sync_cls_avg_factor = sync_cls_avg_factor
        class_weight = loss_cls.get('class_weight', None)
        if class_weight is not None and (self.__class__ is DETRHeadML):
            ...
            class_weight = torch.ones(self.attribute_classes + 1) * class_weight
            # set background class as the last indice
            class_weight[self.attribute_classes] = bg_cls_weight
        ...
        self.loss_cls_2 = MODELS.build(loss_cls)

        self.loss_bbox = MODELS.build(loss_bbox)
        self.loss_iou = MODELS.build(loss_iou)

        if self.loss_cls.use_sigmoid:
            self.cls_out_channels = self.attribute_classes
        else:
            self.cls_out_channels = self.attribute_classes + 1
    ...
    def _init_layers(self) -> None:
        ...
        self.fc_cls_2 = Linear(self.embed_dims, self.cls_out_channels)
    ...
    def forward(self, hidden_states: Tensor) -> Tuple[Tensor]:
        ...
        """
        # TODO: marwan here they compute the loss
        layers_cls_scores = self.fc_cls(hidden_states)
        layers_cls_scores_2 = self.fc_cls_2(hidden_states)

        layers_bbox_preds = self.fc_reg(
            self.activate(self.reg_ffn(hidden_states))).sigmoid()
        return layers_cls_scores, layers_cls_scores_2, layers_bbox_preds
    ...
    def loss_by_feat(
        ...
        # TODO add 2nd class
        losses_cls, losses_cls_2, losses_bbox, losses_iou = multi_apply(
            self.loss_by_feat_single,
            all_layers_cls_scores,
            all_layers_cls_scores_2,
        ...
        loss_dict['loss_cls_2'] = losses_cls_2[-1]

        loss_dict['loss_bbox'] = losses_bbox[-1]
        loss_dict['loss_iou'] = losses_iou[-1]
        # loss from other decoder layers
        num_dec_layer = 0
        for loss_cls_i, loss_cls_2_i, loss_bbox_i, loss_iou_i in \
                zip(losses_cls[:-1], losses_cls_2[:-1], losses_bbox[:-1], losses_iou[:-1]):
            loss_dict[f'd{num_dec_layer}.loss_cls'] = loss_cls_i
            loss_dict[f'd{num_dec_layer}.loss_cls_2'] = loss_cls_2_i
            loss_dict[f'd{num_dec_layer}.loss_bbox'] = loss_bbox_i
            loss_dict[f'd{num_dec_layer}.loss_iou'] = loss_iou_i
            num_dec_layer += 1
        return loss_dict

    def loss_by_feat_single(self, cls_scores: Tensor, cls_scores_2: Tensor, bbox_preds: Tensor,
                            batch_gt_instances: InstanceList,
                            batch_img_metas: List[dict]) -> Tuple[Any, Any, Any, Any]:
        ...
        cls_scores_list = [torch.cat([cls_scores[i], cls_scores_2[i]], dim=1) for i in range(num_imgs)]

        # TODO fix train_size
        # print(batch_img_metas)
        # print('*'*10)
        # print(batch_img_metas[1]['img_id'] <= 20)
        ...
        # else:
        # print('before 1', labels.unique())
        labels[labels == 2] = 1
        labels[labels == 3] = 1
        labels[labels == 4] = 2
        # print('after 1', labels.unique())
        ...
        # print(labels_mask.unique())
        # print('before 2', labels_mask.unique())

        label_weights_mask = torch.cat(
            [label_temp if mask_source[i] else  torch.zeros_like(label_temp) for i, label_temp in enumerate(label_weights_list) ], 0)

        cls_scores_2 = cls_scores_2.reshape(-1, self.cls_out_channels)
        labels_mask[labels_mask == 2] = 0
        labels_mask[labels_mask == 4] = 2
        labels_mask[labels_mask == 3] = 1

        # print('after 2', labels_mask.unique())
    ...
    def _get_targets_single(self, cls_score: Tensor,
                            bbox_pred: Tensor,
    ...
    def predict_by_feat(self,
                        ...
                        all_layers_cls_2_scores: Tensor,
        ...
        cls_scores_2 = all_layers_cls_2_scores[-1]
        ...
        for img_id in range(len(batch_img_metas)):
            ...
            cls_score_2 = cls_scores_2[img_id]

            bbox_pred = bbox_preds[img_id]
            img_meta = batch_img_metas[img_id]

            results_2 = self._predict_by_feat_single(cls_score_2, bbox_pred,
                                                     img_meta, rescale)

            results = self._predict_by_feat_single(cls_score, bbox_pred,
                                                   img_meta, rescale)
            result_list.append(results)
            result_list_2.append(results_2)
    ...
    def _predict_by_feat_single(self,
        ...
        if self.loss_cls.use_sigmoid:
            ...
            det_labels = indexes % self.attribute_classes
            bbox_index = indexes // self.attribute_classes
