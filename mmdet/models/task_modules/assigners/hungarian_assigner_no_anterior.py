...
class HungarianAssignerNoAnterior(BaseAssigner):
    ...
    def assign(self,
        ...
        # TODO add another identifier in json instead of the number of training images in the json
        if img_meta['img_id'] <= 90:
            assigned_gt_inds[:] = 0

            x1_left = torch.min(gt_instances.bboxes[:, 0])
            x2_right = torch.max(gt_instances.bboxes[:, 2])

            y1_top = torch.min(gt_instances.bboxes[:, 1])
            y1_bottom = torch.max(gt_instances.bboxes[:, 3])

            do_not_care = (pred_instances.bboxes[:, 0] < x1_left) | (pred_instances.bboxes[:, 2] > x2_right)
            do_not_care_2 = (pred_instances.bboxes[:, 1] > y1_top) * (pred_instances.bboxes[:, 3] < y1_bottom)

            assigned_gt_inds[do_not_care*do_not_care_2] = -1
