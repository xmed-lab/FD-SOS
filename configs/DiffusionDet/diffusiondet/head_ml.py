...
class DynamicDiffusionDetHeadML(nn.Module):
    ...
    def forward(self, features, init_bboxes, init_t, init_features=None):
        ...
        for head_idx, single_head in enumerate(self.head_series):
            # TODO change here class_logits to return two class_logits
            class_logits, class_logits_2, pred_bboxes, proposal_features = single_head(
                features, bboxes, proposal_features, self.roi_extractor, time)

            if self.deep_supervision:
                inter_class_logits.append(class_logits)
                inter_class_logits_2.append(class_logits_2)

                inter_pred_bboxes.append(pred_bboxes)
            bboxes = pred_bboxes.detach()

        if self.deep_supervision:
            return torch.stack(inter_class_logits), torch.stack(inter_class_logits_2), torch.stack(
                inter_pred_bboxes)
        else:
    ...
    def loss(self, x: Tuple[Tensor], batch_data_samples: SampleList) -> dict:
        ...
        pred_logits, pred_logits_2, pred_bboxes = self(x, batch_diff_bboxes, batch_time)

        output = {
            'pred_logits': pred_logits[-1],
            'pred_logits_2': pred_logits_2[-1],
        ...
        if self.deep_supervision:
            output['aux_outputs'] = [{
                ...
                'pred_logits_2': c,
                'pred_boxes': b
            } for a, b, c in zip(pred_logits[:-1], pred_bboxes[:-1], pred_logits_2[:-1])]
...
@MODELS.register_module()
class SingleDiffusionDetHeadML(nn.Module):
    ...
    def __init__(
        ...
        if self.use_focal_loss or self.use_fed_loss:
            self.class_logits = nn.Linear(feat_channels, 2)
            self.class_logits_2 = nn.Linear(feat_channels, 2)

        else:
            self.class_logits = nn.Linear(feat_channels, 2 + 1)
            self.class_logits_2 = nn.Linear(feat_channels, 2)
    ...
    def forward(self, features, bboxes, pro_features, pooler, time_emb):
        ...
        class_logits_2 = self.class_logits_2(cls_feature)
        ...
        return (class_logits.view(N, num_boxes,
                                  -1), pred_bboxes.view(N, num_boxes,
                                                        -1),
                class_logits_2.view(N, num_boxes,
                                    -1), pred_bboxes.view(N, num_boxes,
                                                          -1)
                , obj_features)
